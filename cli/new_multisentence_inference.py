...
def main():
    ...
    portuguese_sentences_short = [
        # # 1. Intercalação de Termos Técnicos Simples
        # "Eu preciso de um <|language_en|> feedback <|language_pt|> sobre o novo <|language_en|> layout <|language_pt|> do site. O <|language_en|> deadline para a entrega do projeto é amanhã.",

        # # 1. Intercalação de Termos Técnicos Simples
        # "Eu preciso de um feedback sobre o novo layout do site. O deadline para a entrega do projeto é amanhã.",

        # # 2. Frase Iniciada em um Idioma e Finalizada em Outro
        # "Eu estava planejando o nosso fim de semana e pensei: what about a movie night?",

        # # 3. Nomes Próprios e Títulos de Mídia
        # "Meu amigo me recomendou a série 'The Crown' na Netflix, mas eu ainda estou assistindo 'Game of Thrones'. O spoiler que recebi estragou tudo.",

        # # 4. Expressão Idiomática e Gíria
        # "Ele chegou na reunião com uma atitude cool, super relaxado, e disse: 'take it easy', pessoal, vai dar tudo certo.",

        # # 5. Pergunta e Resposta com Troca de Código
        # "Quando me perguntaram sobre o status do projeto, eu respondi: 'It's a work in progress', mas estamos avançando bem.",

        # # 6. Termos de Tecnologia e Redes Sociais
        # "Você pode me mandar o link por direct message? Eu preciso fazer o upload do arquivo no cloud server antes do fim do dia.",

        # # 7. Mistura Complexa no Meio da Frase
        # "A gente precisa marcar um happy hour para celebrar, mas first things first: vamos terminar este relatório.",

        # # 8. Verbo em Inglês com Conjugação em Português
        # "Eu preciso que você printe a tela e me envie. Depois, não se esqueça de logoffar da sua conta.",

        # # 9. Comentário sobre Comida e Bebida
        # "Para o almoço, eu pedi um cheeseburger com batata frita e um milkshake de morango. Estava so good!",

        # # 10. Frase Enfatizando uma Emoção em Inglês
        # "Eu terminei o livro e, oh my God, o final foi absolutamente chocante. Fiquei em estado de shock por vários minutos.",
        "Vai.",
        "Pare agora.",
        "Corra rápido.",
        "Pule alto.",
    ...
    # portuguese_sentences_short = portuguese_sentences_short[:5]
    # portuguese_sentences_mid = portuguese_sentences_mid[:5]
    # portuguese_sentences_mid_long = portuguese_sentences_mid_long[:5]
    # portuguese_sentences_long = portuguese_sentences_long[:5]
    ...
    for length, sentences in tqdm(lists_of_sentences):
        for idx, sentence in tqdm(enumerate(sentences)):
            ...
            output_path = os.path.join(save_dir, f"{language}_{length}_{idx}.wav")
            with torch.no_grad():
                # try:
                wav = model.inference(
                    sentence,
